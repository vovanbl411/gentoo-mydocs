...
| Ветка | `main` |
| Рабочее дерево | Starlight adaptation: Gates 1–5A.1 CLOSED (Gate 4 — `25b5280`; Gate 5A — `292886a`, workflow run #3: build/deploy success, 105 pages; Gate 5A.1 i18n sync guard — `20c5d61`, workflow run #4: check:i18n=PASS, build=success, deploy=success, 105 pages, Pagefind/sitemap success; сайт live: <https://vovanbl411.github.io/gentoo-mydocs/>). Gate 5B.1 — полный English-перевод разделов Installation и Desktop (5 новых переводов, итого 9 EN / 43 fallback) — выполнен 2026-09-25 локально, НЕ закоммичен; deployment не проверен до будущего commit/push и успешного workflow run |
