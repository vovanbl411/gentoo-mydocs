...
| Ветка | `main` |
| Рабочее дерево | Starlight adaptation: Gates 1–5A CLOSED (Gate 4 — sidebar IA/Actions/README — закоммичен как `25b5280`, workflow проверен владельцем 2026-09-25: build/deploy success, Node 24, 53 pages, Pagefind/sitemap success; Gate 5A — English i18n foundation + pilot — закоммичен как `292886a`, workflow run #3: build=success, deploy=success; 105 pages, Pagefind/sitemap success, multilingual deployment live: <https://vovanbl411.github.io/gentoo-mydocs/>). Gate 5A.1 — i18n sync guard (`scripts/check-i18n-sync.mjs`, `npm run check:i18n` внутри `npm run build`) — реализован 2026-09-25 локально, НЕ закоммичен; CI-поведение не проверено до будущего commit/push и успешного workflow run |
