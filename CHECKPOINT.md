...
| Ветка | `main` |
| Рабочее дерево | Starlight adaptation: Gates 1–4 CLOSED (Gate 4 — sidebar IA/Actions/README — закоммичен как `25b5280`, workflow проверен владельцем 2026-09-25: build/deploy success, Node 24, 53 pages, Pagefind/sitemap success; сайт <https://vovanbl411.github.io/gentoo-mydocs/>). Gate 5A (English i18n foundation + pilot: locale `en` + `/en/`-маршруты, 4 переведённые страницы, штатный fallback на русский, sidebar labels через per-item `translations`) выполнен 2026-09-25 локально, НЕ закоммичен; multilingual deployment считаем непроверенным до commit/push и успешного workflow run |
